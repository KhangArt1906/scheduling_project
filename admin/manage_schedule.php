            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <textarea placeholder="Type the title to display on your schedule" class="form-control"
                            name="title" cols="10" rows="2"><?php echo isset($title) ? $title : '' ?></textarea>
                    </div>

                    <div class="form-group">
                        <label for="" class="control-label">Subject</label>
                        <select onchange="fetch_description(this.value)" name="subject" id="subject"
                    ...
                    <div class="form-group">
                        ...
                        <select name="schedule_type" id="" class="custom-select">
                            <option value="1"
                                <?php echo isset($schedule_type) && $schedule_type == 1 ? 'selected' : ''  ?>>Class
                            </option>
                            <option value="2"
                                <?php echo isset($schedule_type) && $schedule_type == 2 ? 'selected' : ''  ?>>Meeting
                            </option>
                            <option value="3"
                                <?php echo isset($schedule_type) && $schedule_type == 3 ? 'selected' : ''  ?>>Others
                            </option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="" class="control-label">Description</label>
                        <textarea placeholder="Automatically Fill by Selecting Subject" id="description"
                            class="form-control" name="description" cols="10"
                            rows="2"><?php echo isset($description) ? $description : '' ?></textarea>
                    </div>


                    <div class="form-group">
                        <label for="" class="control-label">Room No</label>
                        <select required name="room_no" id="room_no" class="custom-select select2">
                    ...
                    <div class="form-group">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" value="1" id="is_repeating"
                                name="is_repeating"
                                <?php echo isset($is_repeating) && $is_repeating != 1 ? '' : 'checked' ?>>
                    ...
                    <div class="form-group for-repeating">
                        ...
                        <select name="dow[]" id="dow" class="custom-select select2" multiple="multiple">
                            ...
                            <option value="<?php echo $i ?>"
                                <?php echo isset($dow_arr) && in_array($i,$dow_arr) ? 'selected' : ''  ?>>
                                <?php echo $dow[$i] ?></option>
                            <?php endfor; ?>
                        </select>
                    </div>
                    <div class="form-group for-repeating">
                        <label for="" class="control-label">Month From</label>
                        <input type="month" name="month_from" id="month_from" class="form-control"
                            value="<?php echo isset($start) ? date("Y-m",strtotime($start)) : '' ?>">
                    </div>
                    <div class="form-group for-repeating">
                        <label for="" class="control-label">Month To</label>
                        <input type="month" name="month_to" id="month_to" class="form-control"
                            value="<?php echo isset($end) ? date("Y-m",strtotime($end)) : '' ?>">
                    </div>
    ...
    $.ajax({
        ...
        success: function(resp) {
            if (resp == 1) {
                ...
                $('#msg').html('<div class="alert alert-danger">Schedule already exist</div>')
                end_load()
            }
...
function fetch_description(val) {
    $.ajax({
        type: "post",
        url: "./fetch.php",
        data: {
